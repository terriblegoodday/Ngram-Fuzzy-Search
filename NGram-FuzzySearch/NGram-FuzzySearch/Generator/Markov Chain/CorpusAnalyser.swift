...
class CorpusAnalyser {
    ...
    func buildMatrix(completion: (_ matrix: [Substring : Occurrence<Substring>]) -> Void) {
        guard let strings = self.parser.parse(fileName: self.corpusName) else {
            return
        }
        var occurrences = [Substring: Occurrence<Substring>]()
    ...
    func nextWord(matrix: [Substring: Occurrence<Substring>], currentWord: Substring, selection: Selection) -> Substring? {
